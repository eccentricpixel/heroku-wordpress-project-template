...
add_action('wp_enqueue_scripts', 'load_appBase_scripts',0);
...
function appBase_menus() {

	register_nav_menus(
		array(
			'header-menu-left' => __( 'Header Menu (left)', 'appBase' ),
			'header-menu-right' => __( 'Header Menu (right)', 'appBase' ),
			'footer-menu' => __( 'Footer Menu', 'appBase' )
		)
	);

}
add_action( 'init', 'appBase_menus' );
...
if ( ! function_exists( 'appBase_entry_meta' ) ) :
/**
 * Prints HTML with meta information for current post: categories, tags, permalink, author, and date.
 *
 * Create your own appBase_entry_meta() to override in a child theme.
 *
 * @since AppBase 2.1.2
 */
function appBase_entry_meta() {
	// Translators: used between list items, there is a space after the comma.
	$categories_list = get_the_category_list( __( ', ', 'appBase' ) );

	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', __( ', ', 'appBase' ) );
	...
	$author = sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
		...
		esc_attr( sprintf( __( 'View all posts by %s', 'appBase' ), get_the_author() ) ),
		get_the_author()
	);

	// Translators: 1 is category, 2 is tag, 3 is the date and 4 is the author's name.
	if ( $tag_list ) {
		$utility_text = __( 'This entry was posted in %1$s and tagged %2$s on %3$s<span class="by-author"> by %4$s</span>.', 'appBase' );
	} elseif ( $categories_list ) {
		$utility_text = __( 'This entry was posted in %1$s on %3$s<span class="by-author"> by %4$s</span>.', 'appBase' );
	} else {
		$utility_text = __( 'This entry was posted on %3$s<span class="by-author"> by %4$s</span>.', 'appBase' );
	}
